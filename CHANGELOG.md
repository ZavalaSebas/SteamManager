...
- **Game title wraps to two lines**: Changed header to a 3-column Grid (accent bar | 300px title | stats) so long game names wrap at 300px (`TextWrapping="Wrap"`, `MaxHeight="72"`) without pushing the progress circle off-screen.
- **Achievements text size reduced**: "X / Y achievements" text decreased from 16px to 14px to match the percentage font size and prevent right-edge clipping.
